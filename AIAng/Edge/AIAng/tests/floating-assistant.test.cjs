...
test('floating assistant switches modes, selects contextual actions, and preserves existing workflows', {
  ...
  try {
    ...
    assert.equal((await launcher.boundingBox()).x + 32, (await page.locator('article').boundingBox()).x);
    ...
    for(const mode of ['board','post','write']) {
      ...
      for(const floating of ['off','on']) {
        ...
        const badge=page.locator('.aiang-chat-modal .aiang-review-badge');
        assert.match(await badge.getAttribute('src'),/AIAng\.png$/);
        await page.evaluate(()=>testHold=true);
        await chips.first().click();await requested('CHAT');
        assert.match(await badge.getAttribute('src'),/AIAng\.gif$/,`${mode}/${floating}: reading animates the shared header`);
        await page.evaluate(()=>releaseTestRequests());
        ...
        assert.match(await badge.getAttribute('src'),/AIAng\.png$/);
        states.push({before,after:await chips.allTextContents()});
        // Follow-up chips also animate, and cancelling restores the static header.
        await chips.nth(1).click();
        assert.match(await badge.getAttribute('src'),/AIAng\.gif$/);
        await page.locator('.aiang-chat-action-chip.is-reading').click();
        assert.match(await badge.getAttribute('src'),/AIAng\.png$/);
        // Free-form questions use the same activity lifecycle in both modes.
        await page.locator('.aiang-chat-input').fill('더 자세히 알려 주세요');
        await page.locator('.aiang-chat-send').click();
        assert.match(await badge.getAttribute('src'),/AIAng\.gif$/);
        await page.evaluate(()=>releaseTestRequests());
        await page.waitForFunction(()=>!document.querySelector('.aiang-chat-send').disabled);
        assert.match(await badge.getAttribute('src'),/AIAng\.png$/);
        await page.evaluate(()=>testFail=true);
        await page.locator('.aiang-chat-input').fill('오류 후에도 멈추나요?');
        await page.locator('.aiang-chat-send').click();
        await page.waitForFunction(()=>!document.querySelector('.aiang-chat-send').disabled);
        assert.match(await badge.getAttribute('src'),/AIAng\.png$/);
        await close();
    ...
    for(const mode of ['post','write','board']) {
      ...
      for(const position of ['left','center','right']) {
        ...
        await page.waitForFunction(({selector,position})=>{
          ...
          const expected=position==='center'?(content.left+content.right)/2:position==='left'?content.left:content.right;
          return Math.abs(button.left+button.width/2-expected)<1;
        },{selector,position});
      ...
      await page.waitForFunction(selector=>Math.abs(document.querySelector('.aiang-floating-launcher').getBoundingClientRect().right-32-document.querySelector(selector).getBoundingClientRect().right)<1,selector);
    ...
    // On a desktop with no outer space, keep the whole button inside the column.
    await page.setViewportSize({width:720,height:844});await go('');
    await page.addStyleTag({content:'body{padding:0}main{box-sizing:border-box;width:100%;max-width:none;margin:0;padding:18px}'});
    for(const position of ['left','right']) {
      await settings({floatingAssistantPosition:position});
      await page.waitForFunction(position=>{
        const b=document.querySelector('.aiang-floating-launcher').getBoundingClientRect();
        return Math.abs(position==='left'?b.left-18:b.right-702)<1;
      },position);
    }
    // Persisted center settings must also work on initial page load.
    await page.goto(url+'?position=center');await launcher.waitFor();
    assert.equal(await page.locator('.aiang-floating').getAttribute('data-position'),'center');
    ...
    for(const position of ['left','center','right']) {
      ...
      if(position==='center') assert.equal((await launcher.boundingBox()).x+32,195);
    ...
    // Height changes preserve the horizontal anchor and keep every menu on screen.
    for(const viewport of [{width:1200,height:900},{width:390,height:380}]) {
      await page.setViewportSize(viewport);await go('?height=high');
      assert.equal(await page.locator('.aiang-floating').getAttribute('data-height'),'high');
      for(const position of ['left','center','right']) {
        await settings({floatingAssistantPosition:position,floatingAssistantHeight:'default'});
        await page.waitForFunction(position=>{
          const root=document.querySelector('.aiang-floating');return root.dataset.position===position&&root.dataset.height==='default';
        },position);
        const baseline=await launcher.boundingBox();
        for(const [height,rise] of [['slight',48],['high',96],['default',0]]) {
          await settings({floatingAssistantHeight:height});
          await page.waitForFunction(height=>document.querySelector('.aiang-floating').dataset.height===height,height);
          const b=await launcher.boundingBox(),m=await menu.boundingBox();
          assert.equal(b.x,baseline.x);
          assert.equal(b.y,baseline.y-rise);
          assert.ok(m.y>=0&&m.y+m.height<=viewport.height);
        }
      }
    }
    await page.setViewportSize({width:390,height:280});await settings({floatingAssistantHeight:'high'});
    await page.waitForFunction(()=>document.querySelector('.aiang-floating').dataset.height==='high');
    assert.equal((await launcher.boundingBox()).y,158,'short viewports reduce the rise');
    assert.ok((await menu.boundingBox()).y>=0);
    // Button size keeps the existing large default and updates the anchor/menu spacing.
    for (const [size, pixels] of [['small',48],['medium',56],['large',64]]) {
      await settings({floatingAssistantSize:size});
      await page.waitForFunction(size=>document.querySelector('.aiang-floating').dataset.size===size,size);
